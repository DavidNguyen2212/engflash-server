...
import { OAUTH_CONFIG } from './constant/oauth.constant';
import { GoogleTokenResponse, GoogleUserInfo } from './interface';
...
export class OauthService {
    ...
    private async getGoogleOAuthToken(code: string) {
        const body = {
            ...
            redirect_uri: this.configService.get('GOOGLE_AUTHORIZED_REDIRECT_URI'),
            grant_type: 'authorization_code'
        }

        const response$ = this.httpService.post('https://oauth2.googleapis.com/token', body,
            {
                headers: {
            ...
            catchError((error: AxiosError) => {
                throw new BadGatewayException('Failed to get OAuth token');
            }),
        )
    ...
    private async directLogin(user: User, req: Request, new_user: boolean) {
        const ef_rf_token = this.authService.generateRfToken(user);
        const hashed = createHash('sha256').update(ef_rf_token).digest('hex');
        ...
        return {
          ef_ac_token: this.authService.generateToken(user),
          ef_rf_token: ef_rf_token,
