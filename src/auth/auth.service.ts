...
import { LoginDto, SignupDto, VerifyCodeDto, ResetPasswordDto } from './dto';
...
export class AuthService {
  ...
  async verifyEmail(verifyCodeDto: VerifyCodeDto) {
    const user = await this.usersService.findByEmailOrThrow(
      verifyCodeDto.email,
    );
  ...
  async refreshToken(refreshToken: string): Promise<BasicAuthResult> {
    ...
    const { isTokenValid, value: storedRaw } = await this.validateRefreshToken(
      oldKey,
      refreshToken,
    );
    ...
    const newPayload = this.jwtService.decode(newRfToken);
  ...
  async handleLogOut(refreshToken: string) {
    ...
    const user = await this.usersService.findByIdOrThrow(payload.sub);
    const key = `refresh:${user.id}:${payload.jti}`;
    const { isTokenValid, value } = await this.validateRefreshToken(
      key,
      refreshToken,
    );
  ...
  async validateRefreshToken(
    key: string,
    token: string,
  ): Promise<{ isTokenValid: boolean; value?: string }> {
    // Case 1: Key disappeared in Redis
    const stored = await this.redis.get(key);
    if (!stored) return { isTokenValid: false };
